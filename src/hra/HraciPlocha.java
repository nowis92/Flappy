...
public class HraciPlocha extends JPanel {
	...
	private void makeLabels() {
		...
		lbZprava.setForeground(Color.WHITE); // barva zpravy na konci a na
												// zacatku
	...
	public void paint(Graphics g) {
		...
		g.drawImage(imgPozadi, posunPozadiX, 0, null); // nalepeni prvniho
														// obrazku
		g.drawImage(imgPozadi, posunPozadiX + imgPozadi.getWidth(), 0, null); // druhy
																				// obrazek
																				// za
																				// nej
	...
	private void posun() {
		if (!pauza && hraBezi) { // hra pobezi a nebude pauza
			...
			if (jeZedKolize(predchoziZed, hrac) || jeZedKolize(aktualniZed, hrac) || jeHraciPlochaKolize(hrac)) {
				konecHryReset();
	...
	private void spustHru() {
		casovacAnimace = new Timer(20, new ActionListener() { // po 20ms se
																// refreshne
			@Override
	...
	public void pripravHraciPlochu() {
		...
		this.addMouseListener(new MouseAdapter() {
			...
			public void mousePressed(MouseEvent e) {
				if (e.getButton() == MouseEvent.BUTTON1) { // stisk leveho
															// tlacitka
					hrac.skok();
				}
